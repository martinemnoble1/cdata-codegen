...
# Qt imports - only needed for GUI classes, not for CCP4Table/CCP4LogToEtree
try:
    from matplotlib.backends.qt_compat import QtCore, QtGui, QtWidgets
    from matplotlib.backends.backend_qt5agg import FigureCanvas
    QT_AVAILABLE = True
except ImportError:
    # Qt not available - GUI classes will fail but parser functions will work
    QT_AVAILABLE = False
    QtCore = QtGui = QtWidgets = FigureCanvas = None
...
def getMatplotlibFontList():
    if not QT_AVAILABLE:
        raise ImportError("Qt not available - getMatplotlibFontList requires Qt")
...
# Qt-dependent GUI classes - only define if Qt is available
if QT_AVAILABLE:
