...
def generate_report_registry(lookup: Dict[str, Any], output_path: str):
    ...
    # Add metadata for each report
    for task_name, report_data in sorted(lookup.items()):
        metadata = {k: v for k, v in report_data.items()
                   if k not in ('module', 'class', 'module_file')}
        metadata['_import_module'] = report_data['module']
        metadata['_import_class'] = report_data['class']
